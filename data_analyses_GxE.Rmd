...
F_GH_ds_surv <- F_GH_ds %>%
  filter(line != "DU17" & env != "plot_3",
         line != "FZ17" & env != "plot_4")
```
...
# model summary
# summary(glm1)
ANODEV_shoot <- Anova(glm1, type = 3)
```

### glm2: survival

```{r survival}
ggplot(data=F_GH_ds_surv, aes(x=survival)) + geom_density() + facet_grid(env ~.)

# prob dist
descdist(F_GH_ds_surv$survival, discrete = TRUE)
# normal, poisson, negative binomial options
fit.poiss <- fitdist(F_GH_ds_surv$survival, "pois")
fit.norm <- fitdist(F_GH_ds_surv$survival, "norm")
fit.nbinom <- fitdist(F_GH_ds_surv$survival, "nbinom")
plot(fit.norm)
plot(fit.nbinom)
plot(fit.poiss)
fit.poiss$aic 
...
testOverdispersion(simuOut_glm2) # NS
...
plot(simOut_glm3) # OK
testOverdispersion(simOut_glm3) # NS

## model summary
# summary(glm3)
ANODEV_leaf <- Anova(glm3, type = 3)
```
...
plot(simOut_glm4) # OK
testOverdispersion(simOut_glm4) # NS
...
# model summary
summary(glm7)
ANODEV_flo <- Anova(glm7, type = 2) # no interaction term, type 2 used
```
...
# summary(glm8_bin)
ANODEV_fru_bin <- Anova(glm8_bin, type = 2) # no interaction term, type 2 used
```
...
write.csv(F_GH_ds_surv, "./dataset_cleaned/survival_cleaned.csv", row.names = FALSE)
